...
impl IntegrationTestClient {
    ...
    pub async fn new(config: IntegrationTestConfig) -> Result<Self> {
        ...
        if balance < ethers::utils::parse_ether("10.0")? {
            ...
            println!(
                "   Using NonceManager for funder account to address {}",
                wallet.address()
            );

            // Use the same contract client's fund_wallet method which uses NonceManager
            match contract_client
                .fund_wallet(wallet.address(), ethers::utils::parse_ether("10.0")?)
                .await
            {
                Ok(tx_hash) => {
                ...
                Err(e) => {
                    println!(
                        "   ⚠️  Funding transaction failed: {}, but continuing...",
                        e
                    );
                }
            }
    ...
    pub async fn test_fid_registration(&self) -> Result<U256> {
        ...
        let fid = self
            .test_real_registration_with_address(random_wallet, test_address)
            .await?;
    ...
    pub async fn test_real_registration_with_address(
        &self,
        wallet: LocalWallet,
        address: Address,
    ) -> Result<U256> {
    ...
    pub async fn test_signer_registration_with_new_wallet(&self) -> Result<SigningKey> {
        ...
        println!("🚀 Registering FID for test address...");
        let fid = self
            .test_real_registration_with_address(random_wallet.clone(), test_address)
            .await?;
        println!("   Registered FID: {}", fid);
        ...
        let registration_result = test_contract_client
            .register_signer_key(
                1, // Ed25519 key type
        ...
        match registration_result {
            ContractResult::Success(_) => {
                ...
                match test_contract_client
                    .key_registry
                    .keys(fid.as_u64(), public_key_bytes.to_vec())
                    .await?
                {
                    ...
                    ContractResult::Error(e) => {
                        ...
                        return Err(anyhow::anyhow!(
                            "Key was not found in registry after registration"
                        ));
                    }
                }
            }
    ...
    pub async fn test_third_party_signer_registration(&self) -> Result<SigningKey> {
        ...
        match tx {
            ...
            None => {
                return Err(anyhow::anyhow!(
                    "ETH transfer to Wallet A failed - no receipt"
                ))
            }
        }
        ...
        let fid = self
            .test_real_registration_with_address(wallet_a.clone(), wallet_a_address)
            .await?;
        println!("   Registered FID: {}", fid);
        ...
        let tx_b = signer_middleware
            .send_transaction(transfer_tx_b, None)
            .await?
            .await?;
        match tx_b {
            Some(receipt) => {
            ...
            None => {
                return Err(anyhow::anyhow!(
                    "ETH transfer to Wallet B failed - no receipt"
                ))
            }
        }
        ...
        let signed_key_request_signature = client_a
            .create_signed_key_request_signature(
                fid.as_u64(),
                wallet_a_address,
                &public_key_bytes,
                deadline,
        ...
        let signed_key_request_metadata = client_a
        ...
        let add_for_signature = client_a
        ...
        let registration_result = client_b
            .submit_signer_registration_with_signatures(
    ...
    pub async fn test_contract_connectivity(&self) -> Result<()> {
        ...
        match self
            .contract_client
            .key_registry
            .total_keys(1339338, 1)
            .await?
        {
            ContractResult::Success(total) => {
                println!(
                    "   ✅ KeyRegistry connected - Total keys for FID 1339338: {}",
                    total
                );
            }
            ContractResult::Error(e) => {
...
async fn test_fid_registration_with_random_address() {
    ...
    let client = IntegrationTestClient::new(config)
        .await
        .expect("Failed to create test client");

    let fid = client
        .test_fid_registration()
        .await
        .expect("FID registration test failed");
    assert!(fid > U256::from(0));
    println!(
        "✅ FID registration with random address successful: {}",
        fid
    );
}
...
async fn test_signer_registration_with_random_address() {
    ...
    let signer_key = client
        .test_signer_registration_with_new_wallet()
        .await
...
async fn test_third_party_signer_registration() {
    ...
    let signer_key = client
        .test_third_party_signer_registration()
        .await
...
async fn test_storage_rental_with_random_address() {
    ...
    client
        .test_storage_rental(fid)
        .await
        .expect("Storage rental test failed");
    println!(
        "✅ Storage rental with random address successful for FID: {}",
        fid
    );
}
...
async fn test_contract_connectivity() {
    ...
    client
        .test_contract_connectivity()
        .await
        .expect("Contract connectivity test failed");
...
async fn test_complete_flow_with_random_address() {
    ...
    let fid = client
        .test_fid_registration()
        .await
        .expect("FID registration failed");
