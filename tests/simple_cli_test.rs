...
async fn test_simple_cli_functionality() {
    ...
    test_cargo_command(
        &["fid", "list"],
        "FID listing",
        |output| output.contains("FID") || output.contains("wallet") || output.contains("No wallet"),
    ).await;
    ...
    for (args, description) in test_cases {
        ...
        match output {
            Ok(output) => {
                ...
                let has_help = stdout.contains("Usage:") || stdout.contains("Commands:") || 
                ...
                if has_help {
                    ...
                    if !stdout.is_empty() {
                        println!("   📝 Output: {}", stdout.lines().take(1).collect::<Vec<_>>().join(" "));
                    }
                }
            }
            Err(e) => {
