...
async fn test_directory_validation(test_data_dir: &str) {
    ...
    assert!(fs::metadata(test_data_dir).is_ok(), "Test directory should exist");
    ...
    println!("   📊 Directory test output: {}", String::from_utf8_lossy(&output.stdout));
...
async fn test_wallet_creation(test_data_dir: &str, _wallet_name: &str) -> bool {
    ...
    if wallet_created {
        ...
        assert!(has_wallets, "Wallet list should show key status information");
...
async fn test_storage_validation(test_data_dir: &str, test_fid: u64) -> Option<String> {
    ...
    let price_output = run_cli_command(test_data_dir, &["storage", "price", &test_fid.to_string(), "--units", "5"]);
...
async fn test_cross_validation(test_data_dir: &str, _wallet_created: bool, price_data: &Option<String>, storage_data: &Option<String>) {
    ...
    for (args, description) in help_commands {
        ...
        assert!(output.status.success(), "{} command should succeed", description);
    ...
    assert!(fid_list_output.status.success(), "FID list command should succeed");
    ...
    assert!(signer_list_output.status.success(), "Signer list command should succeed");
    ...
    if let (Some(fid_price), Some(storage_price)) = (price_data, storage_data) {
        ...
        assert!(fid_price.contains("ETH") || fid_price.contains("price"), "FID price should contain ETH or price info");
        assert!(storage_price.contains("ETH") || storage_price.contains("price"), "Storage price should contain ETH or price info");
...
async fn test_cleanup_validation(test_data_dir: &str, wallet_name: &str) {
    ...
    assert!(!fs::metadata(test_data_dir).is_ok(), "Test directory should be cleaned up");
    ...
    match output {
        Ok(output) => {
            if output.status.success() {
                ...
                Some(std::process::Command::new("anvil")
                    .stdout(Stdio::null())
                    .stderr(Stdio::null())
                    .spawn()
                    .expect("Failed to start Anvil"))
            } else {
                println!("❌ Failed to start Anvil: {}", String::from_utf8_lossy(&output.stderr));
                None
            }
        }
        Err(e) => {
