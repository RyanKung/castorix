...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Storage Registry Paused: {}", status.storage_registry_paused);
    ...
    println!("   Registration Price: {} ETH", ethers::utils::format_ether(registration_price));
    
    let storage_price = client.get_storage_price(1).await?;
    println!("   Storage Price (1 unit): {} ETH", ethers::utils::format_ether(storage_price));
    
    let storage_price_10 = client.get_storage_price(10).await?;
    println!("   Storage Price (10 units): {} ETH", ethers::utils::format_ether(storage_price_10));
    
    // Test wallet client (if we have a private key)
    println!("\n🔑 Wallet Client Demo:");
    let private_key = "0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80";
    if let Ok(wallet) = LocalWallet::from_str(private_key) {
        let provider = ethers::providers::Provider::<ethers::providers::Http>::try_from("http://127.0.0.1:8545")?;
