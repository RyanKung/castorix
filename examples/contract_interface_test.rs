...
use ethers::types::{Address, U256, NameOrAddress, transaction::eip2718::TypedTransaction};
...
async fn main() -> Result<()> {
    ...
    test_function_call(provider, addresses.id_registry, &[0x63, 0x52, 0x21, 0x1e], 1u64).await?;
    
    // Test 2: Alternative ownerOf selector
    println!("\n2. Testing alternative ownerOf selector:");
    test_function_call(provider, addresses.id_registry, &[0x8f, 0x4e, 0xb6, 0x5c], 1u64).await?;
    
    // Test 3: Simple balanceOf call (0x70a08231)
    println!("\n3. Testing balanceOf (0x70a08231):");
    test_function_call(provider, addresses.id_registry, &[0x70, 0xa0, 0x82, 0x31], 1u64).await?;
    ...
    test_function_call(provider, addresses.key_registry, &[0x4f, 0x6c, 0xcc, 0xe7], 1u64).await?;
    
    // Test 2: Alternative key count function
    println!("\n2. Testing alternative key count function:");
    test_function_call(provider, addresses.key_registry, &[0x8f, 0x4e, 0xb6, 0x5c], 1u64).await?;
