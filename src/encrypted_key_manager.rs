...
impl EncryptedKeyManager {
    ...
    pub fn default_config() -> Self {
        ...
        let storage_path = home_dir.join(".castorix").join("keys").to_string_lossy().to_string();
    ...
    pub async fn import_and_encrypt(&mut self, private_key_hex: &str, password: &str, key_name: &str, alias: &str) -> Result<String> {
        ...
        self.key_manager = Some(KeyManager::from_private_key(&hex::encode(private_key_bytes))?);
    ...
    /// # Returns
    /// * `Result<String>` - The wallet address
    pub async fn generate_and_encrypt(&mut self, password: &str, key_name: &str, alias: &str) -> Result<String> {
    ...
    pub async fn load_and_decrypt(&mut self, password: &str, key_name: &str) -> Result<()> {
        ...
        let wallet = LocalWallet::from(signing_key);
        let address = wallet.address();
        let expected_address: Address = encrypted_data.address.parse()
    ...
    fn encrypt_private_key(&self, private_key: &[u8], password: &str, alias: &str) -> Result<EncryptedKeyData> {
        // Generate salt
        ...
        let encrypted_key = cipher.encrypt(&nonce, private_key)
    ...
    fn decrypt_private_key(&self, encrypted_data: &EncryptedKeyData, password: &str) -> Result<Vec<u8>> {
        // Parse salt
        ...
        let encrypted_key = general_purpose::STANDARD.decode(&encrypted_data.encrypted_key)
            .with_context(|| "Failed to decode encrypted key")?;
        
        // Decrypt the private key
        let cipher = Aes256Gcm::new(key);
        let decrypted_key = cipher.decrypt(nonce, encrypted_key.as_ref())
            .map_err(|e| anyhow::anyhow!("Failed to decrypt private key - wrong password?: {}", e))?;
    ...
    fn save_encrypted_key(&self, key_name: &str, encrypted_data: &EncryptedKeyData) -> Result<()> {
        ...
        fs::write(&key_path, json)
            .with_context(|| "Failed to write encrypted key file")?;
    ...
    fn load_encrypted_key(&self, key_name: &str) -> Result<EncryptedKeyData> {
        ...
        let json = fs::read_to_string(&key_path)
            .with_context(|| "Failed to read encrypted key file")?;
        let encrypted_data: EncryptedKeyData = serde_json::from_str(&json)
            .with_context(|| "Failed to parse encrypted key file")?;
        Ok(encrypted_data)
    }
    ...
    /// # Returns
    /// * `Result<()>` - Success or error
    pub async fn rename_key(&mut self, old_name: &str, new_name: &str, password: &str) -> Result<()> {
        ...
        match temp_manager.load_and_decrypt(password, old_name).await {
            ...
            Err(_) => {
                Err(anyhow::anyhow!("Wrong password! Key not renamed."))
            }
        }
    }
    ...
    /// # Returns
    /// * `Result<()>` - Success or error
    pub async fn update_alias(&mut self, key_name: &str, new_alias: &str, password: &str) -> Result<()> {
...
mod tests {
    ...
    async fn test_encrypted_key_generation() {
        ...
        let address = manager.generate_and_encrypt(password, key_name, "test_alias").await.unwrap();
    ...
    async fn test_wrong_password() {
        ...
        // Generate and encrypt key
        manager.generate_and_encrypt(password, key_name, "test_alias").await.unwrap();
