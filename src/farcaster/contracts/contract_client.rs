...
use crate::farcaster::contracts::{
    bundler_abi::BundlerAbi, id_gateway_abi::IdGatewayAbi, id_registry_abi::IdRegistryAbi,
    key_gateway_abi::KeyGatewayAbi, key_registry_abi::KeyRegistryAbi,
    nonce_manager::NonceRegistry,
    storage_registry_abi::StorageRegistryAbi, signed_key_request_validator_abi::SignedKeyRequestValidatorAbi,
    types::{ContractAddresses, Fid, ContractResult},
...
impl FarcasterContractClient {
    ...
    pub fn new(rpc_url: String, addresses: ContractAddresses) -> Result<Self> {
        ...
        let nonce_registry = GLOBAL_NONCE_REGISTRY.get_or_init(|| {
            Arc::new(tokio::sync::Mutex::new(NonceRegistry::new((*provider).clone())))
        });
        ...
        let storage_registry = StorageRegistryAbi::new((*provider).clone(), addresses.storage_registry)?;
        ...
        let signed_key_request_validator = SignedKeyRequestValidatorAbi::new((*provider).clone(), addresses.signed_key_request_validator)?;
    ...
    pub fn new_with_wallet(rpc_url: String, addresses: ContractAddresses, wallet: LocalWallet) -> Result<Self> {
    ...
    pub async fn get_fid_info(&self, fid: Fid) -> Result<FidInfo> {
        ...
        let custody = match custody_result {
            ...
            ContractResult::Error(e) => return Err(anyhow::anyhow!("Failed to get custody address: {}", e)),
        };

        let recovery = match recovery_result {
            ContractResult::Success(addr) => addr,
            ContractResult::Error(e) => return Err(anyhow::anyhow!("Failed to get recovery address: {}", e)),
        };
    ...
    pub async fn get_registration_price(&self) -> Result<U256> {
        ...
        match result {
            ...
            ContractResult::Error(e) => Err(anyhow::anyhow!("Failed to get registration price: {}", e)),
        }
    }
                    ...
                    Some(receipt) => {
                        ...
                        match self.extract_fid_from_receipt(&receipt) {
                            ...
                            Err(_) => {
                                ...
                                println!("   🔍 Event parsing failed, querying contract for FID...");
                                match self.get_fid_by_address(recovery).await {
                                    ...
                                    Err(e) => Ok(ContractResult::Error(format!("Could not determine FID: {}", e))),
                                }
                            }
                        }
                    }
                    None => Ok(ContractResult::Error("Transaction failed - no receipt".to_string())),
                }
            }
    ...
    pub async fn register_fid_with_storage(&self, recovery: Address, extra_storage: u64) -> Result<ContractResult<(u64, U256)>> {
        let wallet = self.wallet.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Wallet required for FID registration"))?;

        // Get registration price with extra storage
        let price = self.storage_registry.price(1 + extra_storage as u32).await?;
        let price = match price {
            ContractResult::Success(p) => p,
            ContractResult::Error(e) => return Ok(ContractResult::Error(format!("Failed to get price: {}", e))),
        };
        ...
        let call = wallet_contract.method::<_, (U256, U256)>("register", (recovery, U256::from(extra_storage)))?;
        match call.value(price).nonce(nonce).send().await {
            Ok(tx) => {
                let receipt = tx.await?;
                match receipt {
    ...
    pub async fn rent_storage(&self, fid: Fid, units: u64) -> Result<ContractResult<U256>> {
        ...
        match call.value(price).nonce(nonce).send().await {
            ...
            Err(e) => Ok(ContractResult::Error(format!("Storage rental failed: {}", e))),
        }
    }

    /// Extract FID from transaction receipt
    fn extract_fid_from_receipt(&self, receipt: &ethers::types::TransactionReceipt) -> Result<u64> {
        println!("🔍 Looking for Register event in {} logs...", receipt.logs.len());
        
        // Try different possible event signatures
        let possible_signatures = vec![
            ("Register(uint256,address,address)", ethers::utils::keccak256("Register(uint256,address,address)")),
            ("Register(address,address,uint256)", ethers::utils::keccak256("Register(address,address,uint256)")),
            ("Register(address,address)", ethers::utils::keccak256("Register(address,address)")),
            ("IdRegistered(uint256,address,address)", ethers::utils::keccak256("IdRegistered(uint256,address,address)")),
            ("IdRegistered(address,address,uint256)", ethers::utils::keccak256("IdRegistered(address,address,uint256)")),
            ("Transfer(address,address,uint256)", ethers::utils::keccak256("Transfer(address,address,uint256)")),
        ];
        
        for (i, log) in receipt.logs.iter().enumerate() {
            println!("   Log {}: topics={}, data={}", i, log.topics.len(), log.data.len());
            if log.topics.len() > 0 {
            ...
            if log.topics.len() >= 2 {
                ...
                for (sig_name, expected_hash) in &possible_signatures {
                    ...
                    println!("   Checking {}: {} == {}", sig_name, topic0, expected_signature);
                    ...
                    if topic0 == expected_signature {
                        ...
                        if sig_name.contains("uint256") {
                            ...
                            for topic_idx in 1..log.topics.len() {
                                ...
                                if fid > 0 && fid < 10000000 { // Reasonable FID range
                                    println!("   📝 Extracted FID from topic {}: {}", topic_idx, fid);
                                    return Ok(fid);
                                }
                            }
                        }
        ...
        Err(anyhow::anyhow!("Could not extract FID from transaction receipt - no matching events found"))
    }

    /// Extract overpayment from transaction receipt
    fn extract_overpayment_from_receipt(&self, _receipt: &ethers::types::TransactionReceipt) -> Result<U256> {
    ...
    async fn get_fid_by_address(&self, address: Address) -> Result<u64> {
        ...
        let call = self.id_registry.contract().method::<_, U256>("idOf", address)?;
    ...
    pub async fn register_signer_key(&self, key_type: u32, key: Vec<u8>, metadata_type: u8, _metadata: Vec<u8>) -> Result<ContractResult<()>> {
        let wallet = self.wallet.as_ref()
        ...
        let fid = match self.address_has_fid(wallet.address()).await? {
            Some(fid) => fid,
            None => return Ok(ContractResult::Error("Wallet address does not have a FID".to_string())),
        };
        ...
        let signature = self.create_signed_key_request_signature(
            fid,
            fid_owner,
            &key,
            deadline,
        ...
        let signed_key_request_metadata = self.create_signed_key_request_metadata(
            fid,
            fid_owner,
            &key,
            deadline,
        ...
        println!("     metadata length: {}", signed_key_request_metadata.len());
        println!("     deadline: {}", deadline);
        
        // Create EIP-712 signature for KeyGateway.addFor
        let add_for_signature = self.create_add_for_signature(
            fid_owner,
            key_type,
            &key,
            metadata_type,
        ...
        let result = self.key_gateway.add_for(
            fid_owner,
            key_type,
            key,
            metadata_type,
        ...
        match result {
            ...
            ContractResult::Error(e) => {
                ...
                Ok(ContractResult::Error(format!("Signer registration failed: {}", e)))
            }
        }
    }
    ...
    pub async fn register_signer_key_for_fid_owner(
        ...
        println!("🔑 Registering signer key for FID owner {} (FID: {})", fid_owner_address, fid);
        ...
        let fid_owner_fid = match self.get_fid_for_address(fid_owner_address).await? {
            ...
            None => return Ok(ContractResult::Error("FID owner address does not have a FID".to_string())),
        };
        ...
        match result {
            ...
            ContractResult::Error(e) => {
                ...
                Ok(ContractResult::Error(format!("Third-party signer registration failed: {}", e)))
            }
        }
    }
    ...
    pub async fn register_signer_key_with_metadata(
        ...
        println!("🔑 Registering signer key with pre-generated metadata for FID owner {} (FID: {})", fid_owner_address, fid);
    ...
    pub async fn create_signed_key_request_metadata(
        ...
        let encoded_metadata = self.signed_key_request_validator.contract()
    ...
    fn create_signed_key_request_typed_data(
        ...
        types.insert("SignedKeyRequest".to_string(), vec![
            Eip712DomainType { name: "requestFid".to_string(), r#type: "uint256".to_string() },
        ...
        message.insert("requestFid".to_string(), serde_json::Value::String(fid.to_string()));
        message.insert("key".to_string(), serde_json::Value::String(format!("0x{}", hex::encode(public_key))));
    ...
    fn create_add_typed_data(
        ...
        let mut message = BTreeMap::new();
        message.insert("owner".to_string(), serde_json::Value::String(format!("0x{:x}", fid_owner)));
