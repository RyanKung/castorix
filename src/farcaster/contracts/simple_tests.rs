...
use crate::farcaster::contracts::{
    client::FarcasterContractClient,
    test_utils::{TestEnvironment, MockContractAddresses, create_funded_wallet},
};
use ethers::{
...
mod tests {
    ...
    async fn test_wallet_funding() -> Result<()> {
        let env = TestEnvironment::new().await?;
        let wallet = create_funded_wallet(&env, U256::from(1000) * U256::from(10).pow(18.into())).await?;
