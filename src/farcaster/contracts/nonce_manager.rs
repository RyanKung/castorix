...
impl NonceManager {
    ...
    pub async fn new(
        ...
        println!("   🔧 Initializing NonceManager for {} with nonce: {}", address, nonce_u64);
    ...
    pub async fn sync_with_blockchain(&self) -> Result<()> {
        let blockchain_nonce = self.provider.get_transaction_count(self.address, None).await?;
        ...
        if blockchain_nonce_u64 > current {
            self.current_nonce.store(blockchain_nonce_u64, Ordering::SeqCst);
            println!("   🔄 Synced nonce for {}: {} -> {}", self.address, current, blockchain_nonce_u64);
        }
    ...
    pub async fn wait_for_nonce_confirmation(&self, target_nonce: U256) -> Result<()> {
        ...
        println!("   ⏳ Waiting for nonce {} to be confirmed for {}", target, self.address);
        ...
        for attempt in 1..=10 {
            ...
            println!("   ⏳ Attempt {}: blockchain nonce {} < target {}", attempt, blockchain_nonce_u64, target);
        }
        
        Err(anyhow::anyhow!("Timeout waiting for nonce {} confirmation", target))
    }
    ...
    pub async fn reset_to_blockchain(&self) -> Result<()> {
        ...
        self.current_nonce.store(blockchain_nonce_u64, Ordering::SeqCst);
        println!("   🔄 Reset nonce for {} to blockchain value: {}", self.address, blockchain_nonce_u64);
