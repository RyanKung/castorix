use crate::{username_proof::{UserNameProof, UserNameType}, key_manager::KeyManager, encrypted_key_manager::EncryptedKeyManager};
use ethers::{
    prelude::*,
    types::Address,
};
...
impl EnsProof {
    ...
    /// # Returns
    /// * `Result<UserNameProof>` - The signed username proof
    pub async fn create_ens_proof_with_wallet(&self, domain: &str, fid: u64, wallet_name: Option<&str>) -> Result<UserNameProof> {
        ...
        let is_owned = match resolved_address {
            Some(addr) => {
                let resolved_addr = Address::from_str(&addr)
                    .with_context(|| "Failed to parse resolved address")?;
        ...
        if !is_owned {
            return Err(anyhow::anyhow!("Domain {} is not owned by the selected wallet address: {:?}", domain, owner_address));
        }
