            ...
            Ok(resolver_data) => {
                if resolver_data.len() == 32 {
                    ...
                    if resolver_address == H160::zero() {
                        ...
                        let mut addr_call_data = hex::decode(addr_selector).map_err(|e| {
                            anyhow::anyhow!("Failed to decode addr selector: {}", e)
                        })?;
                            ...
                            Ok(addr_data) => {
                                if addr_data.len() == 32 {
                                    ...
                                    if resolved_address == H160::zero() {
                                        ...
                                        println!(
                                            "   Result: Resolved address: {resolved_address:?}"
                                        );
                                        Ok(Some(format!("{resolved_address:?}")))
                                    }
                                } else {
                                    println!(
                                        "   Result: Invalid address data length: {}",
                                        addr_data.len()
                                    );
                                    Ok(None)
                                }
                    ...
                    println!(
                        "   Result: Invalid resolver data length: {}",
                        resolver_data.len()
                    );
                    Ok(None)
                }
