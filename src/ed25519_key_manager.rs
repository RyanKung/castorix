...
impl Ed25519KeyManager {
    ...
    pub fn import_key(&mut self, key_name: String, private_key_hex: &str, fid: u64) -> Result<()> {
        ...
        let signing_key = SigningKey::from_bytes(&private_key_bytes[..32].try_into()
    ...
    pub fn get_signing_key(&self, key_name: &str) -> Result<SigningKey> {
        let key_pair = self.keys.get(key_name)
            .ok_or_else(|| anyhow::anyhow!("Key '{}' not found", key_name))?;
        
        Ok(SigningKey::from_bytes(&key_pair.signing_key[..32].try_into()?))
    }
    ...
    pub fn get_verifying_key(&self, key_name: &str) -> Result<VerifyingKey> {
        ...
        Ok(VerifyingKey::from_bytes(&key_pair.verifying_key[..32].try_into()?)?)
    }
...
mod tests {
    ...
    fn test_import_key() {
        ...
        manager.import_key("imported_key".to_string(), &private_key_hex, 456).unwrap();
