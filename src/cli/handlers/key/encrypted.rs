...
pub async fn handle_generate_encrypted() -> Result<()> {
    ...
    // Encrypt and save
    match manager.import_and_encrypt(&hex::encode(private_key_bytes), &password, &key_name, &key_name).await {
        Ok(_) => {
            println!("✅ Encrypted key saved successfully!");
            println!("   Key Name: {}", key_name);
            println!("   Address: {}", address);
            println!("   Storage: ~/.castorix/keys/{}.json", key_name);
        }
...
pub async fn handle_list_keys() -> Result<()> {
    ...
    let manager = EncryptedKeyManager::default();
    match manager.list_keys_with_info() {
        Ok(key_infos) => {
            if key_infos.is_empty() {
                ...
                println!("{:<20} {:<30} {:<42} {}", "Name", "Alias", "Address", "Created");
                println!("{}", "-".repeat(100));
...
pub async fn handle_rename_key(old_name: String, new_name: String) -> Result<()> {
    ...
    if !manager.key_exists(&old_name) {
        println!("❌ Key '{}' not found!", old_name);
        return Ok(());
    }
...
pub async fn handle_import_key() -> Result<()> {
    ...
    match ethers::signers::LocalWallet::from_str(&private_key) {
        Ok(wallet) => {
            ...
            // Encrypt and save
            let mut manager = EncryptedKeyManager::default();
            match manager.import_and_encrypt(&private_key, &password, &key_name, &key_name).await {
                Ok(_) => {
