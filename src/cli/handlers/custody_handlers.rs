        ...
        for entry in entries.flatten() {
            if let Some(file_name) = entry.file_name().to_str() {
                if file_name.starts_with("fid-") && file_name.ends_with("-custody.json") {
                    ...
                    if let Some(fid_str) = file_name.strip_prefix("fid-").and_then(|s| s.strip_suffix("-custody.json")) {
...
async fn handle_custody_import(fid: u64) -> Result<()> {
    ...
    // Confirm password
    let password_confirm = crate::encrypted_key_manager::prompt_password(&format!(
        "Confirm password: "
    ))?;
    
    if password != password_confirm {
        return Err(anyhow::anyhow!("❌ Passwords do not match. Please try again."));
    }
    ...
    let mut encrypted_manager = crate::encrypted_eth_key_manager::EncryptedEthKeyManager::load_from_file(&custody_key_file)?;
...
async fn handle_custody_from_mnemonic(fid: u64) -> Result<()> {
    ...
    encrypted_manager.generate_from_recovery_phrase(fid, &mnemonic, &password).await?;
    ...
    let hub_client = crate::farcaster_client::FarcasterClient::new(config.farcaster_hub_url.clone(), None);
    
    // Get custody address from Hub API
    let actual_custody_address = hub_client.get_custody_address(fid).await
    ...
    if address.to_lowercase() != actual_custody_address.to_lowercase() {
        ...
        return Err(anyhow::anyhow!(
            ...
            address, fid, actual_custody_address,
            fid, actual_custody_address,
            address,
            fid
        ));
    }
