...
fn compile_farcaster_contracts() {
    ...
    match install_result {
        Ok(output) => {
            if !output.status.success() {
                println!("cargo:warning=Failed to install forge dependencies: {}", 
    ...
    match build_result {
        Ok(output) => {
            if !output.status.success() {
                println!("cargo:warning=Failed to compile contracts: {}", 
                        String::from_utf8_lossy(&output.stderr));
            } else {
...
fn generate_rust_bindings() {
    ...
    let contracts = [
        ...
        ("StorageRegistry", "StorageRegistry.sol/StorageRegistry.json"),
        ("IdGateway", "IdGateway.sol/IdGateway.json"),
        ("KeyGateway", "KeyGateway.sol/KeyGateway.json"),
        ("Bundler", "Bundler.sol/Bundler.json"),
        ("SignedKeyRequestValidator", "SignedKeyRequestValidator.sol/SignedKeyRequestValidator.json"),
    ...
    for (contract_name, abi_path) in &contracts {
        ...
        if Path::new(&abi_file).exists() {
            let binding_file = format!("{}/{}_bindings.rs", bindings_dir, contract_name.to_lowercase());
            ...
            match generate_contract_bindings(contract_name, &abi_file, &binding_file) {
                ...
                Err(e) => println!("cargo:warning=Failed to generate bindings for {contract_name}: {e}"),
            }
        } else {
    ...
    let mod_content = contracts.iter()
...
fn generate_contract_bindings(contract_name: &str, abi_file: &str, output_file: &str) -> Result<(), Box<dyn std::error::Error>> {
...
fn setup_test_environment() {
    ...
    let anvil_running = Command::new("curl")
        .args(["-s", "-o", "/dev/null", "-w", "%{http_code}", "http://localhost:8545"])
        .output()
    ...
    if !anvil_running {
        ...
        let _ = Command::new("anvil")
            .args(["--host", "0.0.0.0", "--port", "8545", "--accounts", "10", "--balance", "10000"])
            .spawn();
